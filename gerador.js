...
class GeradorMusical {
    ...
    adaptarMelodia(analiseVoz, tom, acordes) {
        ...
        console.log("Notas detectadas:", notasCantadas);
        ...
        const nomeNota = this.frequenciaParaNota(notaAtual.frequencia);

if (nomeNota) {
    notas.push({
        nota: nomeNota,
        tempo: notaAtual.inicio,
        duracao: notaAtual.fim - notaAtual.inicio
    });
}
...
if (melodia.length > 0 && !analiseVoz.notas) {
    ...
    frequenciaParaNota(frequencia) {
        ...
        for (const [nota, freq] of Object.entries(notas)) {
            ...
            if (diferenca < menorDiferenca && diferenca < freq * 0.12) {
